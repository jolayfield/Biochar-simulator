...
`AtomTyper` (`biochar/opls_typing.py`) assigns each atom an internal OPLS type, and
`GROMACS_OPLS_TYPE_MAP` (`biochar/constants.py`) translates those to the `opls_XXX` names that

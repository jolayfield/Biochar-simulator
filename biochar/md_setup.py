...
class MDSetupConfig:
    ...
    # When `cluster` is set, `setup_one_structure` ALSO writes a
    # `submit_chain.sh` launcher that submits the 7 MD stages as dependent
    # sbatch jobs through the site's GROMACS wrapper script, instead of (or
    # in addition to) the local `run_pipeline.sh`. Currently only "mu3c" is
    # implemented (see `_render_mu3c_chain_script`).
    cluster: Optional[str] = None
    slurm_submit_script: str = "/home0/layfield/gromacs-submits/gromacs_plumed.slurm"
    slurm_sif_file: str = (
        "/home/hope-singularity/image-files/"
        "gromacs-2024-4_plumed-2-9-3_ubuntu-22-04_cuda-12-1-1.sif"
    )
    slurm_partition: str = "mu3c-3G"   # CPU partition; use mu3c-3G-gpu + slurm_gpu=True for GPU
...
def _render_mu3c_chain_script(
    ...
    lines = [
        ...
        "",
        "# ============================================================",
        f"# mu3c SLURM submission chain -- {label}",
        "# Generated by biochar.md_setup. Submits the 7 MD stages as a",
        "# dependency chain through gromacs_plumed.slurm, with one custom",
        "# job (solvate_ions.slurm) filling the editconf/solvate/genion gap.",
...
def setup_one_structure(
    ...
    through both the local `run_pipeline.sh` and (if `config.cluster == "mu3c"`)
    ...
    if cfg.cluster == "mu3c":
        ...
        chain_script = _render_mu3c_chain_script(
        ...
        raise MDSetupError(
            f"Unknown cluster {cfg.cluster!r}; only 'mu3c' is currently implemented."
        )

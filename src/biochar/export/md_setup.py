...
structure produced by a `biochar.workflows.sweep` run.
...
def _render_pipeline_script(
    ...
    *actual* equilibrated structure (via `editconf`/`solvate`/`genion -conc`)
    ...
    script_lines = [
        ...
        "# Generated by biochar.export.md_setup — do not hand-edit box/ion values;",
...
# only). Generated by biochar.export.md_setup; mirrors that script's scratch/results
...
def _render_slurm_chain_script(
    ...
    lines = [
        ...
        "# Generated by biochar.export.md_setup. Submits the 7 MD stages as a",
...
def setup_md_from_manifest(
    ...
    """Read a `biochar.workflows.sweep` manifest.csv and write one MD run directory per row.

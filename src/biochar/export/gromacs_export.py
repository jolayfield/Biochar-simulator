...
class TOPFileWriter:
    ...
    def write(
        ...
        Args:
            ...
            provenance: Lines describing how this structure was produced,
                written as comments under the header. Anything not recoverable
                from the file itself belongs here -- the pH a protonation state
                was sampled at, and the seed it was sampled with.
        """
        with open(filepath, "w") as f:
            # Header
...
class ITPFileWriter:
    ...
    def write(
        ...
        Args:
            ...
            include_dihedrals: If True, include dihedral section
            provenance: Lines describing how this structure was produced,
                written as comments under the header. A protonation state is
                one sample from an ensemble, so the pH and the seed that
                produced it are what make the topology reproducible.
...
class GromacsExporter:
    ...
    def export(
        ...
        Args:
            ...
            title: First line of the `.gro`. The only human-readable line in
                the file, and the one a reader sees first, so it should say
                what distinguishes this structure from a similar one.
            provenance: Comment lines for the `.top` and `.itp` headers,
                recording what produced the structure.

...
def render_condensation_script(
    ...
    header += [
        ...
        "# Generated by biochar.workflows.condensation -- review, then run on your GROMACS build.",
        "# ============================================================",
        "",
        f'GMX="{gmx_bin}"',
...
def generate_and_condense(
    ...
    :class:`biochar.pipeline.biochar_generator.BiocharGenerator` (using *generator_config*,
...
def render_surface_script(
    ...
    lines = [
        ...
        "# (xy at 1 bar, z frozen to keep the gap). Generated by biochar.workflows.condensation.",
        "# ============================================================",
        "",
        f'GMX="{gmx_bin}"',
        'SIM="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"',
...
# --------------------------------------------------------------------------- #
# Phase 5: validation helpers (density / SASA / TEM / convergence)
# --------------------------------------------------------------------------- #
# Wood validated on: true density (free-volume tool, He probe 0.13 nm, over the
# converged 300 K portion; rho_solid = rho_system / (1 - Vf)), normalized SASA
# (N2 probe 0.18 nm, over the last 2 ns; nSASA = SASA / (2*A_xy)), simulated TEM
# (computeM ctem: 200 keV, 0.02 rad aperture, 20 nm defocus), and convergence
# (plateaus in RMS, density, box dimensions). These are analyses of trajectories
# the user has already produced -- so this renders commands, setup-only.
def render_validation_script(
    bulk_prefix: str = "rep_1/final",
    surface_prefix: str = "surf_npt",
    gmx_bin: str = "gmx",
    surface_ns: float = _FINAL_NS,
    sasa_last_ns: float = 2.0,
) -> str:
    """`analyze.sh` — the Wood validation analyses, run *after* the MD produces
    the condensed-bulk (`{bulk_prefix}.*`) and surface (`{surface_prefix}.*`)
    trajectories. Setup-only: emits the `gmx` commands + normalization notes.
    """
    if sasa_last_ns <= 0 or sasa_last_ns > surface_ns:
        raise CondensationError("sasa_last_ns must be in (0, surface_ns]")
    sasa_start_ps = int(round((surface_ns - sasa_last_ns) * 1000))
    lines = [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "",
        "# ============================================================",
        "# Wood et al. 2024 validation analyses (run AFTER the MD)",
        "# Edit the input paths below if your run layout differs.",
        "# Generated by biochar.workflows.condensation.",
        "# ============================================================",
        "",
        f'GMX="{gmx_bin}"',
        'SIM="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"',
        f'BULK="$SIM/{bulk_prefix}"      # condensed-bulk stage (.tpr/.xtc/.edr)',
        f'SURF="$SIM/{surface_prefix}"   # exposed-surface stage (.tpr/.xtc)',
        "",
        "# ---- True density (bulk) ------------------------------------------------",
        "# System density (kg/m^3) over the converged 300 K final NPT:",
        'echo Density | "$GMX" energy -f "$BULK.edr" -o "$SIM/density.xvg"',
        "# Free-volume fraction with a He-sized probe (0.13 nm):",
        '"$GMX" freevolume -s "$BULK.tpr" -f "$BULK.xtc" -radius 0.13 -o "$SIM/freevolume.xvg"',
        '# True (solid) density = rho_system / (1 - Vf_0.13)  -- combine the two above.',
        "",
        "# ---- Surface roughness (SASA) ------------------------------------------",
        f"# N2-sized probe (0.18 nm), averaged over the last {sasa_last_ns:g} ns:",
        f'echo System | "$GMX" sasa -s "$SURF.tpr" -f "$SURF.xtc" -probe 0.18 -b {sasa_start_ps} -o "$SIM/sasa.xvg"',
        '# Normalize: nSASA = <SASA> / (2 * A_xy),  A_xy = box_x * box_y  (2 = both surfaces).',
        "",
        "# ---- Convergence checks -------------------------------------------------",
        'printf "Density\\nBox-Z\\n" | "$GMX" energy -f "$BULK.edr" -o "$SIM/convergence.xvg"',
        '"$GMX" rms -s "$BULK.tpr" -f "$BULK.xtc" -o "$SIM/rmsd.xvg" <<< "System System" || true',
        "# Confirm plateaus in density, box dimensions, and RMSD before trusting the model.",
        "",
        "# ---- Simulated TEM (external tool) -------------------------------------",
        '# computeM `ctem`: beam 200 keV, aperture 0.02 rad, defocus 20 nm',
        'echo "Run computeM ctem on the final structure to compare against experimental HRTEM."',
        "",
    ]
    return "\n".join(lines) + "\n"


def write_validation_setup(
    output_dir: str | Path,
    bulk_prefix: str = "rep_1/final",
    surface_prefix: str = "surf_npt",
    gmx_bin: str = "gmx",
    ...
    """Write ``analyze.sh`` (the Wood validation analyses) into *output_dir*."""
    out = Path(output_dir)
    out.mkdir(parents=True, exist_ok=True)
    script = render_validation_script(
        bulk_prefix=bulk_prefix, surface_prefix=surface_prefix, gmx_bin=gmx_bin,
        surface_ns=surface_ns, sasa_last_ns=sasa_last_ns,
    )
    p = out / "analyze.sh"
    p.write_text(script)
...
# --------------------------------------------------------------------------- #
# Convenience: add surface + validation stages into a condensation run dir
# --------------------------------------------------------------------------- #
def add_surface_and_validation(
    output_dir: str | Path,
    which_repeat: int = 1,
    gap_nm: float = _DEFAULT_GAP_NM,
    surface: bool = True,
    ...
    """Write the surface (Phase 4) and validation (Phase 5) scripts into an
    existing condensation run directory (from :func:`setup_condensation` /
    :func:`generate_and_condense`), wired to that run's own outputs.

    The surface script z-expands ``rep_{which_repeat}/final.gro`` (produced when
    the user runs ``run_condensation.sh``) against the in-dir ``system.top`` — no
    files are copied, since everything lives in the same directory. Setup-only.
    """
    out = Path(output_dir)
    if not (out / "system.top").exists():
        raise CondensationError(
            f"{out}/system.top not found -- run setup_condensation first"
        )
    bulk_prefix = f"rep_{which_repeat}/final"

    if surface:
        (out / "surf_npt.mdp").write_text(_surface_npt_mdp())
        sp = out / "run_surface.sh"
        sp.write_text(render_surface_script(
            f"{bulk_prefix}.gro", "system.top", gap_nm=gap_nm, gmx_bin=gmx_bin, ntomp=ntomp,
        ))
        sp.chmod(sp.stat().st_mode | stat.S_IEXEC | stat.S_IXGRP | stat.S_IXOTH)

    if validation:
        write_validation_setup(
            out, bulk_prefix=bulk_prefix, surface_prefix="surf_npt", gmx_bin=gmx_bin,
        )
    return out

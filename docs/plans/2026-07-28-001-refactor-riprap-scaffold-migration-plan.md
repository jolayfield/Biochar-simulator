...
> in Phase 4 with a real functional payoff. The remaining Phase 4 subpackage split (six new
> subpackages: `pipeline/`, `charges/`, `export/`, `workflows/`, `models/`, `cli/`) is now also
> done, merged in PR #41. Only phase 5 (container, deferred) remains, and it is not blocking.
...
> **Done.** The subpackage split landed as proposed above — `pipeline/`, `charges/`,
> `export/`, `workflows/`, `models/`, `cli/` — with `constants.py` kept at the package root
> since every subpackage imports it. `__all__` and `[project.scripts]` are unchanged; the
> four console entry points now point at `biochar.cli.<module>:main` instead of
> `biochar.<module>:main`, which is an internal packaging detail, not a public-API change.
>
> Two things the plan didn't anticipate: two modules (`temperature_model.py`, `ml_charges.py`)
> compute their bundled-data path as `Path(__file__).parent / "data"`, which silently breaks
> one level deeper once the file gains a subpackage parent — fixed to `.parent.parent`, caught
> by a real `FileNotFoundError` during verification, not by inspection. And roughly 250
> `biochar.<module>` references across tests, docs, README, and docstring cross-references
> needed the same path update; a handful of those were literal provenance strings this project
> writes into *generated* GROMACS run scripts (e.g. `# Generated by biochar.condensation`),
> not just documentation.
>
> Verified with a full isolated-venv wheel install (not just editable), matching the rigor
> used for the Phase 3 src-layout move: import, the four console scripts, and a real
> `biochar-gen` run all work with zero working-tree code on `sys.path`.
